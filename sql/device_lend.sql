...
CREATE TABLE `device_lend` (
  ...
  `lend_time` timestamp NOT NULL COMMENT '借出时间',
  `expect_return_time` timestamp NOT NULL COMMENT '预计归还时间',
  `return_time` timestamp NOT NULL COMMENT '实际归还时间',
  `detail` timestamp NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '借出详情',
